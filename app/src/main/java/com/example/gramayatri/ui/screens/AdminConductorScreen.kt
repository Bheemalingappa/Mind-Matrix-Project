...
fun AdminConductorScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Conductor Management", color = Color.White) },
                navigationIcon = {
...
@Composable
fun ConductorItem(conductor: ConductorModel) {
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(conductor.name, color = Color.White, fontWeight = FontWeight.Bold, fontSize = 16.sp)
                Text("Bus: ${conductor.busNumber} • ${conductor.currentRoute}", color = Color(0xFFCBD5E1), fontSize = 13.sp)
            }
            Box(
                modifier = Modifier
                    .background(
                        if (conductor.status == "ONLINE") Color(0xFF059669) else Color(0xFF64748B),
                        shape = RoundedCornerShape(8.dp)
                    )
                    .padding(horizontal = 8.dp, vertical = 4.dp)
            ) {
                Text(conductor.status, color = Color.White, fontSize = 10.sp, fontWeight = FontWeight.Bold)
            }
        }
    }
}